...
class EditProductViewController: UIViewController {
    ...
    private func configData() {
        ...
        guard let image = imageProduct.image else { return }
        imgString = convertImageToBase64String(img: image)
        nameTextField.text = product?.name ?? ""
        let price = Int(product?.price ?? 0)
        priceTextField.text = String(price)
        quantityTextField.text = String(product?.stock ?? 0)
        descriptionTextView.text = product?.description ?? ""
        guard let categoryItem = product?.category else { return }

        if categoryItem ==  "jacketsCoats"  {
            categoryTempLabel = "Jackets & Coats"
        }
        if categoryItem == "hoodiesSweatshirts"  {
            categoryTempLabel = "Hoodies & Sweatshirts"
        }
        if categoryItem == "cardiganJumpers" {
            categoryTempLabel = "Cardigan & Jumpers"
        }
        if categoryItem == "tshirtTanks"  {
            categoryTempLabel = "T-shirt & Tanks"
        }
        if categoryItem == "shoes"  {
            categoryTempLabel = "Shoes"
        }
        if categoryItem == "shirts" {
            categoryTempLabel = "Shirts"
        }
        if categoryItem == "basics" {
            categoryTempLabel = "Basics"
        }
        if categoryItem ==  "blazersSuits" {
            categoryTempLabel = "Blazers & Suits"
        }
        if categoryItem == "shorts"  {
            categoryTempLabel = "Shorts"
        }
        if categoryItem == "trousers" {
            categoryTempLabel = "Trousers"
        }
        if categoryItem == "jeans" {
            categoryTempLabel = "Jeans"
        }
        if categoryItem == "swimwear" {
            categoryTempLabel = "Swimwear"
        }
        if categoryItem == "underwear" {
            categoryTempLabel = "Underwear"
        }
        if categoryItem == "socks" {
            categoryTempLabel = "Socks"
        }
        
        classifyTemp = product?.classify ?? ""
        categoryTemp = categoryItem
        nameItemOfCategory.text = categoryTempLabel
        nameItemOfClassify.text = product?.classify
                        ...
                        if validateDes() == true {
                            ...
                            if !(categoryTemp == "")  {
                                if !(classifyTemp == "") {
                                    ...
                                    editProduct(productID: product?._id ?? "", price: Double(price) ?? 0.0 , name: name, description: des ?? "", classify: classifyTemp, category: categoryTemp, stock: Int(quantity) ?? 0, imgString: imgString)
...
extension EditProductViewController {
    private func editProduct(productID: String, price: Double, name: String, description: String, classify: String, category: String, stock: Int, imgString: String) {
        viewAnimate.isHidden = false
        APIService.editProduct(productID: productID, price: price, name: name, description: description, classify: classify, category: category, stock: stock, imgString: imgString) { product, error in
